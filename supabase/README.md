...
| 역할 | 할 수 있는 것 |
|---|---|
| `assistant` (조교) | 사진 올리기 · 채점 중단 · 결과 보기 · 문항 고치기 · **확정** |
| `teacher` (선생님) | 조교와 같습니다 (§13.33에서 확정을 조교에게 열었습니다) |
| `admin` | 위 전부 + 직원 관리 · 설정 · 비용 · 모델 비교 |

역할이 실제로 가르는 것은 지금 **관리자냐 아니냐** 하나뿐입니다.
`teacher`는 표시용으로 남겨뒀습니다 — 사용량 화면에서 누가 누구인지 읽는 데
쓰이고, 나중에 다시 갈라야 할 때 칼럼이 없으면 그때 만들어야 합니다.
