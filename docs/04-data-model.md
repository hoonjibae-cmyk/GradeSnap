...
#### 🟢 해법 후보 — 정답지를 함께 업로드해 자동 추출

선생님이 시험지를 만들 때 **정답지도 함께 만든다면**, 두 PDF를 같이 올리는 것만으로
좌표와 정답을 동시에 뽑을 수 있습니다.
...
정답지 PDF  ─┘        ↓
              달라진 영역 = 답안 영역 좌표
              그 영역의 정답지 텍스트 레이어 = 정답 텍스트
```

두 문서는 레이아웃이 같고 **정답 텍스트만 다르므로**, 차분 영역이 곧 답안 영역입니다.
정답은 PDF 텍스트 레이어에서 그대로 읽으므로 **OCR도 필요 없습니다.**

유니크보카가 정확히 이 구조로 시험지/정답지를 만들고 있습니다
(`.answer-line--ghost` ↔ `.answer-line--filled`). 같은 원리를 PDF 수준에서 쓰는 것입니다.

선생님이 추가로 할 일은 **"정답지도 같이 올리기"** 하나뿐입니다.

> ❓ **확인 필요**: 선생님들이 실제로 정답지를 별도 파일로 만드십니까?
> 만들지 않고 채점을 머릿속으로 하신다면 이 해법은 쓸 수 없고,
> 답안 영역 자동 검출 + 정답 수기 입력으로 가야 합니다.
