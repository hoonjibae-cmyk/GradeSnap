...
import type { AnswerKeyRow, Role, SettingsRow, SheetPageRow, SheetRow, StaffRow, UsageEventRow } from "@/lib/db/schema";
import {
  byHour,
  byStaff,
  DAY_NAMES,
...
function Admin({ db, staff }: { db: SupabaseClient; staff: StaffRow }) {
  ...
  return (
    <main className="mx-auto max-w-4xl p-5 pb-24">
      ...
      <AnswerKeys db={db} onError={setErr} />
 ...
 * **정답지를 시험마다 한 장 등록합니다.**
 *
 * 프로그램은 답안지의 답란만 봅니다. 단어 시험은 그것으로 정답이 나오지만,
 * 순서배열·문장삽입은 정답이 **지문**에 달려 있어 알 수가 없습니다(§13.40).
 * 사람이 한 번 알려주면 그때부터 그 시험은 채점됩니다. 30명이 같은 시험을
 * 보므로 수고는 1/30입니다.
 *
 * 🔴 **읽은 것을 바로 저장하지 않습니다.** 표로 보여주고 사람이 확인한 뒤에
 * 저장합니다. 정답지가 틀리면 그 반 전체가 같은 오류로 채점됩니다.
 */
function AnswerKeys({ db, onError }: { db: SupabaseClient; onError: (m: string) => void }) {
  ...
  return (
    <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4">
      <h2 className="font-semibold">정답지</h2>
      <p className="mt-1 text-sm text-slate-600">
        순서배열·문장삽입처럼 <strong>지문을 봐야 정답이 나오는 문항</strong>은 프로그램이 알 수 없습니다.
        시험마다 정답지를 <strong>한 번</strong> 등록해 두면 그때부터 채점됩니다.
      </p>
          ...
          {keys.map((k) => (
            <li key={k.slug} className="flex items-center justify-between gap-3 py-2 text-sm">
              <span>
                ...
                <span className="ml-2 text-xs text-slate-500">{k.items.length}문항</span>
              </span>
              <button
                onClick={() =>
